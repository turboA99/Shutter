...
public class AutomaticVirtualCursor : MonoBehaviour
{
    ...
    [SerializeField] Physics2DRaycaster raycaster;
    ...
    void CheckControlScheme()
    {
        switch (_playerInput.currentControlScheme)
        {
            ...
            case "Gamepad":
                ...
                break;
            default:
                throw new Exception("Unknown control scheme " + _playerInput.currentControlScheme);
        }
    }
    ...
    /// <summary>
    /// Determines which cursor representation to use. If this is set to <see cref="VirtualMouseInput.CursorMode.SoftwareCursor"/>
    ...
    /// cref="VirtualMouseInput.CursorMode.HardwareCursorIfAvailable"/> and there is a native <see cref="Mouse"/> device present,
    ...
    ///
    /// Note that if <see cref="VirtualMouseInput.CursorMode.HardwareCursorIfAvailable"/> is used, the software cursor is still used
    ...
    public VirtualMouseInput.CursorMode cursorMode
    {
        ...
        set
        {
            ...
            if (m_CursorMode == VirtualMouseInput.CursorMode.HardwareCursorIfAvailable && m_SystemMouse != null)
            {
            ...
            if (m_CursorMode == VirtualMouseInput.CursorMode.HardwareCursorIfAvailable)
    ...
    /// <remarks>
    /// If <see cref="cursorMode"/> is set to <see cref="VirtualMouseInput.CursorMode.HardwareCursorIfAvailable"/>, this graphic will
    /// be disabled.
    ///
    ...
    /// </remarks>
    /// <seealso cref="VirtualMouseInput.CursorMode.SoftwareCursor"/>
    ...
    protected void OnEnable()
    {
        ...
        if (m_StickActionTriggeredDelegate == null)
            m_StickActionTriggeredDelegate = OnStickActionTriggered;
        SetActionCallback(m_StickAction, m_StickActionTriggeredDelegate, true);
    ...
    protected void OnDisable()
    {
        ...
        if (m_StickActionTriggeredDelegate != null)
        {
            SetActionCallback(m_StickAction, m_StickActionTriggeredDelegate, false);
        }
    ...
    private void UpdateMotion()
    {
        ...
        else
        {
            ...
            InputEvent inputEvent = new InputEvent(new FourCC('M', 'O', 'U', 'S'), 8, virtualMouse.deviceId);
            unsafe
            {
                InputEventPtr inputEventPtr = new InputEventPtr(&inputEvent);
            }
            ...
            virtualMouse.WarpCursorPosition(newPosition);

            // Update software cursor transform, if any.
            if (m_CursorTransform != null &&
                (m_CursorMode == VirtualMouseInput.CursorMode.SoftwareCursor ||
                 (m_CursorMode == VirtualMouseInput.CursorMode.HardwareCursorIfAvailable && m_SystemMouse == null)))
    ...
    [SerializeField] private VirtualMouseInput.CursorMode m_CursorMode;
    ...
    private Action<InputAction.CallbackContext> m_StickActionTriggeredDelegate;
    ...
    private void OnStickActionTriggered(InputAction.CallbackContext context)
    {
        if (m_VirtualMouse == null)
            return;

        m_VirtualMouse.CopyState<MouseState>(out var mouseState);
        mouseState.position = virtualMouse.position.ReadValue();

        InputState.Change(m_VirtualMouse.position, mouseState.position);
    }
